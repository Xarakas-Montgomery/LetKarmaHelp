...
function validateCred(card) {
  ...
  function findInvalidCards(cards) {
    // Array to store invalid credit card numbers
    const invalidCards = [];
  
    // Loop through each card in the nested array
    for (let card of cards) {
      // Use the validateCred() function to check if the card is invalid
      if (!validateCred(card)) {
        // If invalid, add the card to the invalidCards array
